...
public class APIRestController {
    ...
    @GetMapping("/StoredDiets") //Revisable**********
    public ResponseEntity<List<Diet>> getStoredDiets(HttpServletRequest request){
            List<Diet> dietas = this.currentUser.getStoredDiets();
    ...
    @GetMapping("/DietMaker")//Revisable **************
    public ResponseEntity<List<Menu>> getDiet_Maker(HttpServletRequest request){
        List<Menu> menuAll = menuRepository.findAll();
        List<Menu> menuContains = new ArrayList<>();
        for (Menu menu: menuAll){
            boolean conteins = false;
            ...
            if(!conteins){
                menuContains.add(menu);
                return new ResponseEntity<>(menuContains, HttpStatus.OK);
            }
        }
        return new ResponseEntity<>(dietCreate, HttpStatus.OK);
    }

    @PostMapping("/processRemoveDiet/{id}")//Delete pal final
    ...
    public List<Menu> addDiet(@PathVariable long id){
    ...
    @PostMapping("/processFormDiet")//Revisalbe ***********
    @ResponseStatus(HttpStatus.CREATED)
    public Diet addFormDiet(@RequestParam String name){
        Diet dietNew = new Diet(name, dietCreate);
        dietRepository.save(dietNew);
        currentUser.addStoredDiets(dietNew);
        userRepository.save(currentUser);
    ...
    @GetMapping("/diet/{id}/{nombre}")//Revisalbe*************
    public ResponseEntity<Optional<Diet>> getDietPage(@PathVariable long id){
        Optional<Diet> d = dietService.findById(id);
        List<Menu> menuList = new ArrayList<>();
        if (d.isPresent()){
            for (Menu m : d.get().getDieta()){
