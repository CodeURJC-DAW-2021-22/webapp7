...
export class MenuactiveComponent implements OnInit {
  ...
  constructor(private loginService: LoginService, userService:UsersService,recipeService:RecipesService, menuService: MenuService) {
    ...
    this.doc.text('Email: ' + this.user.mail, 20, 43);
        ...
        this.barChartData = [
          { data: [ this.vegetablesMenu, this.vegetablesStandard, this.proteinMenu, this.proteinStandard, this.hydratesMenu, this.hydratesStandard, this.carboHydratesMenu, this.carboHydratesStandard, this.highInFatMenu, this.highInFatStandard], label: 'How healthy is your menu?',
          }
        ];
